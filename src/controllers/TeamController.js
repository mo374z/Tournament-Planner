...
const {
  ...
  checkLoginStatus,
  isAdmin,
} = require("../middleware/auth"); // Pfad zur auth.js-Datei
...
router.use((req, res, next) => {
  // Middleware, um Benutzerinformationen an res.locals anzuhängen
...
router.get("/clearTeamCounters", isAdmin, async (req, res) => {
  //Clear Team Counters only for Admins
  try {
  ...
  return Promise.all(
    games.map(async game => {
      ...
      return {
        ...game._doc,
        opponent: opponent,
        formattedTime: new Date(game.time).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
      };
...
async function getGoalsForTeam(teamId) {
  const games = await Game.find({
    "goalsLog.teamIndex": { $exists: true },
  }).exec();
  const goals = [];
  let teamGoalIndex = 0;
    ...
    for (const goal of game.goalsLog) {
      ...
      if (teamIdFromGame === teamId.toString()) {
        ...
        const player = goal.player
          ? await Player.findById(goal.player).exec()
          : null;
        const opponents = await Promise.all(
          game.opponents.map(async opponentId => {
            const team = await Team.findById(opponentId).exec();
        ...
        goals.push({
          ...
          opponent: opponent,
          formattedTimestamp: new Date(goal.timestamp).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
            second: "2-digit",
          }),
          formatedgameTimestamp: new Date(goal.gameTimestamp * 1000)
            .toISOString()
            .slice(14, 19),
